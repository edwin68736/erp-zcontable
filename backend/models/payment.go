...
type Payment struct {
	...
	// GORM (ninguna consulta de lectura/reporte usa .Unscoped(); el único .Unscoped() sobre Payment
	// en todo el backend es el guard de idempotencia interno de DeletePaymentTx y, desde Fase 7,
	// PaymentService.ListVoided — la vía de auditoría dedicada para consultar anulados). Estos 3
	// campos son estrictamente aditivos: motivo, actor y una marca de tiempo con semántica de
	// "anulación", que DeletedAt no provee. Nullable: nil = pago nunca anulado (incluye TODOS los
	// pagos históricos, sin backfill — un pago histórico soft-eliminado antes de Fase 6 queda con
	// DeletedAt fijado pero VoidedAt=nil a propósito, decisión explícita, ver
	// docs/diseno-fase6-paso2-cancelaciones-writeoff-2026-09-15.md E.3: no se inventan datos
	// históricos que no existen).
	...
	// VoidedByUser (Fase 7, docs/diseno-fase7-paso2-ui-reportes-2026-09-15.md C): mismo patrón que
	// TukifacFiscalReceipt.IssuedByUser — solo se popula cuando el llamador lo precarga
	// explícitamente (PaymentService.ListVoided); el resto de consultas de Payment no lo necesitan.
	VoidedByUser         *User                 `gorm:"foreignKey:VoidedBy" json:"voided_by_user,omitempty"`
