...
type Document struct {
	...
	// OriginSettlementID: liquidación que ORIGINALMENTE creó esta deuda (INMUTABLE, se fija una sola
	// vez al crear el Document en createSettlementDebtDocument y nunca vuelve a cambiar). Distinto de
	// TaxSettlementID, que sí cambia cuando la deuda se arrastra de una liquidación a otra. Nil cuando
	// el Document no nació de una liquidación (manual, suscripción) o cuando el origen histórico no
	// pudo reconstruirse con evidencia suficiente (ver backfill en document_migrations.go). Es la
	// fuente de verdad de "pertenencia" usada por IsSettlementOwnedDebt — Blueprint Fase 1 §9.
	OriginSettlementID   *uint      `gorm:"index" json:"origin_settlement_id,omitempty"`
