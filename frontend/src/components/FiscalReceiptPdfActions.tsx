...
import { downloadFiscalReceiptPdf, type ReceiptPdfFormat } from '../pdf/fiscalReceiptPdf';
...
const FiscalReceiptPdfActions = ({ receiptId, compact }: Props) => {
  const [busy, setBusy] = useState<'a4' | 'a5' | 'ticket' | null>(null);
  const [viewer, setViewer] = useState<{ open: boolean; format: ReceiptPdfFormat }>({
    open: false,
    format: 'a4',
  });

  const runDownload = async (format: ReceiptPdfFormat) => {
    try {
      setBusy(format);
    ...
    <>
      <div className={`flex flex-wrap items-center gap-1.5 ${compact ? '' : 'gap-2'}`}>
        ...
        <button
          type="button"
          disabled={busy !== null}
          onClick={() => setViewer({ open: true, format: 'a5' })}
          className={btnClass}
          title="Ver PDF A5"
        >
